...
class Simulator:
    ...
    """
    def __init__(self, data_dict, ac_dict, ob_dict, initial_shares, look_back, price_smooth):
        self.data_dict = data_dict
        self.price_smooth = price_smooth

        self.hothead = 'False'
        self.trading_steps = 10
    ...
    def reset(self, day, session):
        ...
        self.data = self.data_dict[day][session]
        ...
        self.initial_loc = self.trading_steps
        self.current_loc = self.trading_steps
        ...
        self.bid_price_sequence = [[]] * self.max_level
        self.ask_price_sequence = [[]] * self.max_level
        self.OrderBook = OrderBook(self.get_historical_order())
        while self.current_loc <= self.trading_steps:
            self.OrderBook.update(self.get_historical_order())
            for level in np.arange(1, self.max_level):
    ...
    def step(self, action, ac = False):
        ...
        elif self.current_loc >= (self.trading_steps * 2) -1:
            order_size = - self.inventory
        elif ac:
        ...
        info = {'step': self.current_loc - (self.trading_steps - 1),
    ...
    def observation(self):
        ...
        for i in np.arange(1, self.max_level + 1):
        ...
        for i in np.arange(1, self.max_level + 1):
            ...
            if 'Bid Volume {}'.format(i) in self.ob_dict.keys():
                ...
                bv = (self.OrderBook.getBidsQuantity(i) - (self.initial_inventory / self.trading_steps)) / (self.initial_inventory / self.trading_steps)
                obs.append(bv)
            if 'Ask Volume {}'.format(i) in self.ob_dict.keys():
                # obs.append((self.OrderBook.getAsksQuantity(i) - self.volume_mean) / self.volume_std)
                av = (self.OrderBook.getAsksQuantity(i) - (self.initial_inventory / self.trading_steps)) / (self.initial_inventory / self.trading_steps)
                obs.append(av)
